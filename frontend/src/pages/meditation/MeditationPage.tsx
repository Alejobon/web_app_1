// MeditationPage — mood selection → phrases displayed with fade → ambient experience.
...
import {
  ...
  type MeditationMood,
} from "@/features/meditation/hooks/useMeditationSession";
...
function getSafeBackTarget(from: unknown) {
  if (
    typeof from === "string" &&
    from.startsWith("/") &&
    !from.startsWith("//") &&
    !from.startsWith("/p/meditation")
  ) {
    return from;
  }
...
export function MeditationPage() {
  ...
  const canStart = selectedMood !== null && selectedDuration !== null;
  const backTarget = getSafeBackTarget(
    (location.state as MeditationRouteState | null)?.from,
  );
  ...
  function handleStartSession() {
    if (!selectedMood || selectedDuration === null) return;
    void session.startSession(selectedMood, selectedDuration);
  }
  ...
  if (session.phase === "select") {
    return (
      <MeditationBackdrop>
        <button
          onClick={handleBack}
          className="absolute left-4 top-4 z-30 rounded-full bg-card/55 p-2 text-foreground shadow-sm backdrop-blur transition hover:bg-card/80"
          aria-label="Volver"
          type="button"
        >
            ...
            <div className="grid w-full grid-cols-2 gap-3">
              {MOODS.map((m) => (
                <button
                  ...
                  className={cn(
                    "flex flex-col items-center gap-2 rounded-2xl border-2 p-5 transition hover:scale-105",
                    m.color,
                    selectedMood === m.id &&
              ...
              {DURATIONS.map((d) => (
                <Button
                  ...
                  className={cn(
                    ...
                    selectedDuration !== d && "bg-background/70",
                  )}
  ...
  return (
    <MeditationBackdrop fixed>
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <div
          className="absolute right-[20%] top-[30%] h-2 w-2 animate-pulse rounded-full bg-secondary/20"
          style={{ animationDelay: "1s" }}
        />
        <div
          className="absolute bottom-[25%] left-[25%] h-2 w-2 animate-pulse rounded-full bg-accent/20"
          style={{ animationDelay: "2s" }}
        />
        <div
          className="absolute bottom-[30%] right-[15%] h-3 w-3 animate-pulse rounded-full bg-primary/15"
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center px-8 text-center">
        {session.phrases.length > 0 &&
          session.currentPhrase < session.phrases.length && (
            <p
