...
import { ApiError, apiClient, apiUrl, getAccessToken } from "@/lib/api-client";
...
export async function streamChatMessage({ chatId, content, historyLimit, signal, onToken, onDone, onError }: StreamChatInput) {
  const token = await getAccessToken();
  if (!token) throw new ApiError("Sesión requerida", 401);
  const body = historyLimit === undefined ? { content } : { content, historyLimit };
  const response = await fetch(apiUrl("/chats/" + segment(chatId) + "/messages/stream"), { method: "POST", headers: { "Content-Type": "application/json", Accept: "text/event-stream", Authorization: "Bearer " + token }, body: JSON.stringify(body), signal });
